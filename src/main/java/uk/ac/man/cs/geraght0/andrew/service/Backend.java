...
public class Backend {
  ...
  public Optional<String> checkForNewerVersion() {
    ...
    final String ver = buildProperties.getVersion();
    if (!NumberUtils.isParsable(ver)) {
      ...
      try {
        ...
        final ArrayNode json = rest.getForObject(url, ArrayNode.class);
        if (json != null) {
          final Optional<JsonNode> max = Streams.stream(json.elements())
          ...
          if (!max.isPresent()) {
            ...
            final String version = max.get()
                                      .get("name")
                                      .asText();
            final double latestVersion = Double.parseDouble(version);
